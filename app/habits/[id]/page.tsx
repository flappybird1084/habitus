...
import { getColor, formatFrequency, todayISO, getPastDays } from "@/lib/models";
...
export default function HabitDetailPage() {
  ...
  const getHabitWithStats = useHabitStore((s) => s.getHabitWithStats);
  const toggleEntry = useHabitStore((s) => s.toggleEntry);
  const deleteHabit = useHabitStore((s) => s.deleteHabit);
  const archiveHabit = useHabitStore((s) => s.archiveHabit);
  const firstWeekday = useHabitStore((s) => s.firstWeekday);
  // Direct subscriptions so the component re-renders when data changes.
  // Calling getHabitWithStats/getEntries as plain functions (not selectors)
  // doesn't register a reactive dependency, so toggleEntry/archiveHabit
  // changes would never be reflected without a reload.
  const habits = useHabitStore((s) => s.habits);
  const entries = useHabitStore((s) => s.entries[id] ?? []);

  // useMemo serves double duty: makes habits/entries explicit deps (no lint
  // warning) and avoids rerunning computeScore/computeStreaks on unrelated
  // state changes like chartView toggles.
  const habit = useMemo(
    () => getHabitWithStats(id),
    [getHabitWithStats, id, habits, entries]
  );
  ...
  return (
    <div className="min-h-full">
      ...
      <div className="max-w-3xl mx-auto px-4 py-4 space-y-4 animate-fade-in">
        ...
        <div
          className="card p-5"
          style={{ borderTop: `3px solid ${hexColor}` }}
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <p className="text-3xl font-bold text-[var(--text-primary)]">
                {formatPercent(habit.completionRate)}
              </p>
        ...
        <div className="card p-4">
          ...
          <HistoryGrid
            ...
            onCellClick={(date) => toggleEntry(id, date)}
          />
        </div>
