...
import { setFirstWeekday } from "@/lib/actions/settings";
import { clearAllData } from "@/lib/actions/habits";
import { PageHeader } from "@/components/PageHeader";
import { cn } from "@/lib/utils";
import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import type { Habit, Entry } from "@/lib/models";
...
export function SettingsClient({ habits, entries, firstWeekday }: Props) {
  ...
  const handleSetFirstWeekday = (day: number) => {
    startTransition(async () => {
      await setFirstWeekday(day);
      router.refresh();
    });
  };
    ...
    <div className="min-h-full">
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-6 animate-fade-in">
              ...
              <div className="grid grid-cols-3 gap-2">
                {themeOptions.map(({ value, label, icon: Icon }) => (
                  ...
                  >
                    <Icon
                      size={18}
                      className={cn(theme === value ? "text-emerald-500" : "text-[var(--text-muted)]")}
                    />
                    <span
                      className={cn(
                        "text-xs font-medium",
        ...
        <section>
          ...
          <div className="card divide-y divide-[var(--border)]">
            <div className="p-4">
              <p className="text-sm font-medium text-[var(--text-primary)] mb-3">First day of week</p>
              <div className="grid grid-cols-3 gap-2">
                {WEEKDAY_OPTIONS.map(({ label, value }) => (
                  <button
                    key={value}
                    onClick={() => handleSetFirstWeekday(value)}
                    disabled={isPending}
                    className={cn(
